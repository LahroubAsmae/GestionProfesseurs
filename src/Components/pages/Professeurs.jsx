    ...
    <div className="flex justify-center items-center min-h-screen">
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-lg w-full bg-white shadow-md rounded-lg p-6">
        <div className="mb-4">
          ...
          <input {...register("firstName", { required: "Le nom est obligatoire" })} className="w-full border rounded-md p-2" />
          {errors.firstName && <p className="text-red-500 text-sm">{errors.firstName.message}</p>}
        </div>

        <div className="mb-4">
          <label className="block text-gray-700 font-medium">Prénom *</label>
          <input {...register("lastName", { required: "Le prénom est obligatoire" })} className="w-full border rounded-md p-2" />
          {errors.lastName && <p className="text-red-500 text-sm">{errors.lastName.message}</p>}
        </div>

        <div className="mb-4">
          <label className="block text-gray-700 font-medium">Email *</label>
          <input type="email" {...register("email", { required: "Champ requis" })} className="w-full border rounded-md p-2" />
          {errors.email && <p className="text-red-500 text-sm">{errors.email.message}</p>}
        </div>

        <div className="mb-4">
          <label className="block text-gray-700 font-medium">Téléphone *</label>
          <input {...register("phone", { required: "Champ requis", pattern: /^\+?[0-9]{10}$/ })} className="w-full border rounded-md p-2" placeholder="06********" />
        ...
        <div className="mb-4">
          ...
          <div className="flex space-x-4">
            <label className="flex items-center">
              <input type="radio" value="permanent" {...register("status", { required: "Champ requis" })} className="mr-2" /> Permanent
            </label>
            <label className="flex items-center">
              <input type="radio" value="vacataire" {...register("status", { required: "Champ requis" })} className="mr-2" /> Vacataire
            </label>
          </div>
